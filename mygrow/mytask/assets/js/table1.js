...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function filterTableByToday() {
        console.log('Filtering table1 for today:', formattedToday);
    ...
    try {
        const savedContent = await IndexedDBService.loadTable('table1');
        if (savedContent) {
            console.log('Loaded table1 content:', savedContent);
            fullTableContent = savedContent;
            table.html(savedContent);
            createHiddenTable();
            filterTableByToday();
        } else {
            console.log('No saved content for table1');
            createHiddenTable();
            // No "No data" placeholder
        }
    } catch (error) {
        console.error('Error loading table1:', error);
        createHiddenTable();
    }
    ...
    function debouncedSave() {
        ...
        saveTimeout = setTimeout(async () => {
            ...
            console.log('Before save table1:', currentContent);
            if (!currentContent || currentContent.trim() === '') {
                console.log('Table1 empty, skipping save');
                return;
            }
            ...
            console.log('Saving table1:', fullTableContent);

            try {
                await IndexedDBService.saveTable('table1', fullTableContent);
                console.log('Saved table1 successfully');
                filterTableByToday();
                table.find('tr.notes :input').hide();
                ...
                if (activeCell && range) {
                    ...
                    if (newCell) {
                        ...
                        newCell.focus();
                        console.log('Cursor restored in table1');
                    }
                }
            } catch (error) {
                console.error('Error saving table1:', error);
            }
        }, 250);
    }

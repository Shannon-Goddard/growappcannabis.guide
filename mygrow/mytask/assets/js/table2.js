...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function filterTableByToday() {
        console.log('Filtering table2 for today:', formattedToday);
    ...
    try {
        const savedContent = await IndexedDBService.loadTable('table2');
        if (savedContent) {
            console.log('Loaded table2 content:', savedContent);
            fullTableContent = savedContent;
            table.html(savedContent);
            createHiddenTable();
            filterTableByToday();
        } else {
            console.log('No saved content for table2');
            createHiddenTable();
            // No "No data" placeholder
        }
    } catch (error) {
        console.error('Error loading table2:', error);
        createHiddenTable();
    }
    ...
    function debouncedSave() {
        ...
        saveTimeout = setTimeout(async () => {
            ...
            console.log('Before save table2:', currentContent);
            if (!currentContent || currentContent.trim() === '') {
                console.log('Table2 empty, skipping save');
                return;
            }
            ...
            console.log('Saving table2:', fullTableContent);

            try {
                await IndexedDBService.saveTable('table2', fullTableContent);
                console.log('Saved table2 successfully');
                filterTableByToday();
                table.find('tr.notes :input').hide();
                ...
                if (activeCell && range) {
                    ...
                    if (newCell) {
                        ...
                        newCell.focus();
                        console.log('Cursor restored in table2');
                    }
                }
            } catch (error) {
                console.error('Error saving table2:', error);
            }
        }, 250);
    }

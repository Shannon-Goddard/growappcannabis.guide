...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function filterTableByToday() {
        console.log('Filtering table3 for today:', formattedToday);
    ...
    try {
        const savedContent = await IndexedDBService.loadTable('table3');
        if (savedContent) {
            console.log('Loaded table3 content:', savedContent);
            fullTableContent = savedContent;
            table.html(savedContent);
            createHiddenTable();
            filterTableByToday();
        } else {
            console.log('No saved content for table3');
            createHiddenTable();
            // No "No data" placeholder
        }
    } catch (error) {
        console.error('Error loading table3:', error);
        createHiddenTable();
    }
    ...
    function debouncedSave() {
        ...
        saveTimeout = setTimeout(async () => {
            ...
            console.log('Before save table3:', currentContent);
            if (!currentContent || currentContent.trim() === '') {
                console.log('Table3 empty, skipping save');
                return;
            }
            ...
            console.log('Saving table3:', fullTableContent);

            try {
                await IndexedDBService.saveTable('table3', fullTableContent);
                console.log('Saved table3 successfully');
                filterTableByToday();
                table.find('tr.notes :input').hide();
                ...
                if (activeCell && range) {
                    ...
                    if (newCell) {
                        ...
                        newCell.focus();
                        console.log('Cursor restored in table3');
                    }
                }
            } catch (error) {
                console.error('Error saving table3:', error);
            }
        }, 250);
    }
